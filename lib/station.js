...
var DEFAULT_BOG_OVERRIDES = {
  ...
  foxPort: '/Services/FoxService',
  foxsPort: '/Services/FoxService'
};
 ...
 * service
 * @param {Array.<String>} [config.jvmArgs] a list of additional JVM arguments
 * to pass to `station.exe`
...
Station.prototype.start = function (cb) {
  ...
  function doStart() {
    ...
    var spawnArgs = [ stationName ].concat(systemProperties, jvmArgs);
    process = spawn(cmd, spawnArgs, config);
